...
const { readDestinationIp, shouldHairpinToLocalStack } = require('./routing')
const { isDirectoryFrame, decodeHubDirectory } = require('./hub-directory')
const { encodeZ32 } = require('./key-encoding')
const { connectAsClient } = require('./dht-connect')
 ...
 * (HyperDHT hub mode; same transport as {@link startServer}). This is the same path ambient mesh
...
function createClient (opts) {
  ...
  const localMeshIpv4Str = stripHostFromCidr(ip)
  const localMeshIpv6Str = ipv6 ? stripHostFromCidr(String(ipv6)) : null
  ...
  tun.on('data', function (packet) {
    ...
    const hairpinCtx = {
      localKey: connectOpts.keyPair.publicKey,
      ka: kaClient,
      localMeshIpv4: localMeshIpv4Str,
      localMeshIpv6: localMeshIpv6Str
    }
    if (destIp && shouldHairpinToLocalStack(destIp, hairpinCtx)) {
      try {
        tun.write(packet)
      } catch (_) {}
      return
