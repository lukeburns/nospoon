 ...
 * @param {function(Buffer): boolean} [opts.injectLocalPacket] — if inner IP pkt is for another local TUN (shared swarm), deliver and return true
 */
async function startSwarmMesh (opts) {
  const {
    ...
    injectLocalPacket: injectLocalPacketOpt
  } = opts

  const injectLocalPacket =
    injectLocalPacketOpt && typeof injectLocalPacketOpt === 'function'
      ? injectLocalPacketOpt
      : null
  ...
  function makeTunnelFrameHandler (clientIp, peerKeyHex) {
    return function onTunnelFrame (framedPayload) {
      ...
      if (peerConn) {
        ...
        if (injectLocalPacket && injectLocalPacket(packet)) return
        try {
          tun.write(packet)
        } catch (_) {}
      }
  ...
  async function wireConnection (conn) {
    ...
    const decode = createDecoder(function (framedPayload) {
      if (!authDone) {
        ...
        startKeepalive(conn)
        finalized = true
        byPeerHex.set(peerKeyHex, { conn, clientIp, decode, onData, muted: false })
  ...
  /**
   * Stop feeding this stream into the topic decoder (shared Hyperswarm: primary pool takes over inbound).
   * Outbound topic TUN traffic still uses {@link router#addPeer}; only one stack may `on('data')` the socket.
   * @param {string} peerKeyHex
   */
  function muteInboundDataForPeerKey (peerKeyHex) {
    const h = String(peerKeyHex || '').trim().toLowerCase()
    const state = byPeerHex.get(h)
    if (!state || !state.onData || state.muted) return
    state.conn.removeListener('data', state.onData)
    state.muted = true
  }
