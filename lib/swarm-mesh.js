 ...
 * @param {function(string): import('stream').Duplex|null} [opts.lookupOutboundRelay] — TUN egress: resolve dest IPv4 → peer stream across direct + topic routers (shared Hyperswarm); when set, used instead of this mesh’s `router` alone
 */
async function startSwarmMesh (opts) {
  const {
    ...
    lookupOutboundRelay: lookupOutboundRelayOpt
  } = opts
  ...
  const lookupOutboundRelay =
    typeof lookupOutboundRelayOpt === 'function' ? lookupOutboundRelayOpt : null
  ...
  tun.on('data', function (packet) {
    const destIp = readDestinationIp(packet)
    if (!destIp) return
    const connection = lookupOutboundRelay
