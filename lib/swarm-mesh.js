...
async function startSwarmMesh (opts) {
  ...
  if (swarmOpt) {
    ...
    keyPair = HyperDHT.keyPair(
      seedBuf || Buffer.from(loadOrCreatePersistedSeedHex(), 'hex')
    )
  ...
  tun.on('data', function (packet) {
    ...
    if (connection) {
      ...
      if (!quiet) console.error('[tun-egress] send', destIp, 'len=' + packet.length, 'topic=' + topicIdStr)
    } else if (!quiet) {
      console.error('[tun-egress] no route for', destIp, 'topic=' + topicIdStr)
    }
  })
