 ...
 * @param {{ bindAddress: string, port?: number, controlHost?: string, controlPort: number, onError?: (e: Error) => void }} opts — `controlHost` is where the control HTTP server listens (defaults to loopback; must match when the control plane binds a loopback alias)
 */
function createWhoisAuthProxy (opts) {
  ...
  const controlHost = String(opts.controlHost || '127.0.0.1').trim() || '127.0.0.1'
  ...
  function copyHeaders (req) {
    ...
    out.host = net.isIPv6(controlHost)
