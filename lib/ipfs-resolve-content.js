 ...
 * @param {number} port
 * @param {{ dhtClientMode?: boolean, upnpAutoConfirmAddress?: boolean, appendAnnounce?: string[], announceNoCircuit?: boolean, webrtc?: boolean, extraListenIpv4?: string[] }} [opts]
 ...
 * }} [heliaOpts] — `dhtClientMode` from control UI when set; otherwise `NOSPOON_HELIA_DHT_CLIENT_MODE`. UPnP and append-announce default from env. WebRTC defaults off on Linux (`NOSPOON_HELIA_WEBRTC`). `extraListenIpv4` adds mesh TUN addresses when the primary v4 bind is not wildcard.
...
async function createHeliaBackend (dataDir, heliaOpts) {
  ...
  const libp2p = await nospoonHeliaLibp2pOptions(nospoonHeliaSwarmPort(), {
    dhtClientMode,
    upnpAutoConfirmAddress,
    announceNoCircuit,
  ...
  return {
    ...
    /**
     * Import a directory tree via unixfs.addAll, pin the root, return display label.
     * @param {Array<{ path: string, content: Buffer | Uint8Array }>} entries
     * @returns {Promise<{ cid: string, filename: string }>}
     */
    addDirectory: async function (entries) {
      if (!Array.isArray(entries) || entries.length === 0) {
        throw new Error('no files in directory upload')
      }
      const seen = new Set()
      /** @type {Array<{ path: string, content: Uint8Array }>} */
      const normalized = []
      for (const e of entries) {
        const path = normalizeUnixfsRelPath(e.path)
        if (seen.has(path)) throw new Error('duplicate path: ' + path)
        seen.add(path)
        const buf = e.content
        const u8 = buf instanceof Uint8Array ? buf : Buffer.from(buf)
        normalized.push({ path, content: u8 })
      }
      let last
