 ...
 * @param {number} port
 * @param {{ dhtClientMode?: boolean, upnpAutoConfirmAddress?: boolean, appendAnnounce?: string[], announceNoCircuit?: boolean, webrtc?: boolean }} [opts]
 ...
 *   webrtc?: boolean
 * }} [heliaOpts] — `dhtClientMode` from control UI when set; otherwise `NOSPOON_HELIA_DHT_CLIENT_MODE`. UPnP and append-announce default from env. WebRTC defaults off on Linux (`NOSPOON_HELIA_WEBRTC`).
...
async function createHeliaBackend (dataDir, heliaOpts) {
  ...
  async function mergeMeshHeliaPeer (o) {
    ...
    helia.libp2p.dial(remotePeerId).catch(function () {})
  }
