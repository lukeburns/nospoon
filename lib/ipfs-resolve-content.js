...
async function createHeliaBackend (dataDir) {
  ...
  return {
    ...
    /**
     * Root pins in Helia (one row per uploaded / explicitly pinned DAG).
     * @returns {Promise<Array<{ cid: string, filename: string | null }>>}
     */
    listSeeds: async function () {
    ...
    /**
     * Unpin a root CID and run GC so blocks may be removed from disk.
     * @param {string} cidStr
     */
    unseed: async function (cidStr) {
