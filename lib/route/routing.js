...
// key-address table: destination IP resolves to the peer Ed25519 public key, then we
// look up the live stream by that key (framing uses scoped endpoint ids, not raw keys).
...
/**
 * OS routing can send packets **read** from the TUN (egress from the host into userspace) toward
 * this machine’s own mesh/TUN IP. There is no peer stream for “self”; without handling, those
 * packets were dropped. Inject them back with `tun.write` so the stack delivers them locally
 * (e.g. browser → `http://10.0.0.1` on the primary TUN address).
 *
 * @param {string|null|undefined} destIp — from {@link readDestinationIp}
 * @param {{ localKey: Buffer, ka?: object | null, localMeshIpv4?: string | null, localMeshIpv6?: string | null }} ctx — extend router ctx with optional `localMeshIpv4` / `localMeshIpv6` when `ka` is absent (authenticated server)
 * @returns {boolean}
 */
function shouldHairpinToLocalStack (destIp, ctx) {
  if (!destIp || !ctx || !ctx.localKey) return false
  const { localKey, ka, localMeshIpv4, localMeshIpv6 } = ctx
  if (ka) {
    try {
      return ka.ipToKey(destIp).equals(localKey)
    } catch (_) {}
  }
  if (localMeshIpv4 && destIp === localMeshIpv4) return true
  if (
    localMeshIpv6 &&
    net.isIPv6(destIp) &&
    net.isIPv6(localMeshIpv6) &&
    destIp.toLowerCase() === localMeshIpv6.toLowerCase()
  ) {
    return true
  }
  return false
...
/**
 * Use when **relaying** mesh→mesh: inner IP must look like it came from that peer (IPv4 alias or
 * `ka` mapping to `peerKeyHex`). Do **not** apply to frames you will `tun.write` — internet return
 * traffic has arbitrary inner source addresses (NAT reverse path).
 *
 * IPv6 inner packets are accepted: mesh aliases are v4-only; v6 sources would never equal `peerIpv4`.
 */
function tunnelSourceAllowedForPeerStream (packet, peerIpv4, peerKeyHex, ka) {
  const version = readIpVersion(packet)
  if (version === 6) return true
  if (version !== 4) return false
  if (!peerIpv4 || !peerKeyHex || !ka) return false
  const wantHex = String(peerKeyHex).trim().toLowerCase()
  const srcIp = readSourceIp(packet)
  if (srcIp === peerIpv4) return true
  try {
    return ka.ipToKey(srcIp).toString('hex') === wantHex
  } catch {
    return false
  }
}
