...
/**
 * @param {string} name — normalized FQDN (no trailing dot)
 * @returns {string | null} IPv4 dotted-quad, or null
 */
function parseReverseIPv4 (name) {
  const m = IN_ADDR_ARPA_RE.exec(name)
  if (!m) return null
 ...
 */
function awaitMaybe (v) {
  return v != null && typeof v.then === 'function' ? v : Promise.resolve(v)
}
 ...
 * @param {function({ kind: 'key', keyHex: string } | { kind: 'keyTopic', keyHex: string, topicRef: string }): string | null | Promise<string | null>} [opts.resolveMeshA] — returns IPv4 string or null (or Promise of same; used when manual has no matching RR for that qtype)
 ...
 * @param {function(string): string | null | Promise<string | null>} [opts.reverseLookupPtr] — IPv4 string → FQDN hostname for PTR answers (or null)
...
function createDnsServer (opts) {
  ...
  const reverseLookupPtr = opts.reverseLookupPtr
  ...
  function sendServFail (query, rinfo) {
    const flags = dnsPacket.RECURSION_AVAILABLE | 2
  ...
  async function handleQueryMessage (msg, rinfo) {
    let query
    try {
      query = dnsPacket.decode(msg)
    } catch {
    ...
    if (qtype === 'PTR') {
      const reversedIp = parseReverseIPv4(name)
      if (reversedIp && typeof reverseLookupPtr === 'function') {
        let ptrName = null
        try {
          ptrName = /** @type {string | null} */ (
            await awaitMaybe(reverseLookupPtr(reversedIp))
          )
        } catch (e) {
          if (opts.onError) opts.onError(e instanceof Error ? e : new Error(String(e)))
          sendServFail(query, rinfo)
          return
        }
        if (ptrName) {
          const response = dnsPacket.encode({
    ...
    if (typeof resolveMeshA === 'function' && qtype === 'A') {
      ...
      if (meshId) {
        isMeshQuery = true
        try {
          meshIpv4 = /** @type {string | null} */ (
            await awaitMaybe(resolveMeshA(meshId))
          )
        } catch (e) {
          if (opts.onError) opts.onError(e instanceof Error ? e : new Error(String(e)))
          sendServFail(query, rinfo)
    ...
    if (!hasMeshA && !hasLocalA && !hasLocalAAAA && !hasCidA) {
      // Valid mesh / CID name forms but no A to return: NODATA (NOERROR + empty), not NXDOMAIN —
      // the name is in our namespace; NXDOMAIN would wrongly imply the name does not exist.
  ...
  function onMessage (msg, rinfo) {
    void handleQueryMessage(msg, rinfo).catch(function (e) {
      if (opts.onError) opts.onError(e instanceof Error ? e : new Error(String(e)))
      try {
        const q = dnsPacket.decode(msg)
        if (q.type === 'query' && q.questions && q.questions.length > 0) {
          sendServFail(q, rinfo)
        }
      } catch (_) {}
    })
  }
