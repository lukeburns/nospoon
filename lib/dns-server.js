...
/**
 * @param {object} opts
 * @param {function(string): { ipv4?: string, ipv6?: string } | null} opts.lookupManual — normalized FQDN
 * @param {function({ kind: 'key', keyHex: string } | { kind: 'keyTopic', keyHex: string, topicRef: string }): string | null} [opts.resolveMeshA] — returns IPv4 string or null
 * @param {number} [opts.port=5343]
...
function createDnsServer (opts) {
  ...
  const port = opts.port != null ? Number(opts.port) : 5343
  ...
  function onMessage (msg, rinfo) {
    ...
    if (!hasMeshA && !hasLocalA && !hasLocalAAAA) {
