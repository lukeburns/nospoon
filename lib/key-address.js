 ...
 *   [ IPv4 bytes 0–11 ][ 32-byte src endpoint id ][ 32-byte dst endpoint id ][ IP payload ]
...
function createKeyAddressTable (opts = {}) {
  ...
  function encodeIpv4 (packet) {
    ...
    const srcE = ipToWireEndpoint(packet.subarray(12, 16))
    const dstE = ipToWireEndpoint(packet.subarray(16, 20))
    const prefix = Buffer.from(packet.subarray(0, IPV4_PREFIX_LEN))
    prefix.writeUInt16BE(0, 10)
    return Buffer.concat([prefix, srcE, dstE, packet.subarray(IPV4_HEADER_LEN)])
  }
  ...
  function decodeIpv4 (buf) {
    ...
    const srcIp = endpointIdToIp(buf.subarray(IPV4_PREFIX_LEN, IPV4_PREFIX_LEN + KEY_LEN))
    const dstIp = endpointIdToIp(buf.subarray(IPV4_PREFIX_LEN + KEY_LEN, WIRE_IPV4_KEY_HEADER_LEN))
