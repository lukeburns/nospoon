 ...
 * @param {import('./mesh-identifier').MeshIdentifier} [opts.localMeshId] — e.g. `{ kind: 'keyTopic', keyHex, topicId }` for topic TUN local address
 */
function createKeyAddressTable (opts = {}) {
   ...
   * @param {import('./mesh-identifier').MeshIdentifier} [meshId] — optional 1:1 label for this IP ↔ logical id (DNS / routing)
   */
  function register (ip, key, meshId) {
    ...
    if (meshId != null) {
      ...
      ipToMeshId.set(ipStr, freezeMeshId(meshId))
  ...
  if (opts.localIp != null && opts.localKey != null) {
    register(opts.localIp, opts.localKey, opts.localMeshId)
  }
...
function freezeMeshId (id) {
  if (id.kind === 'key') {
    return { kind: 'key', keyHex: normalizeKeyHexForMesh(id.keyHex) }
  }
  return {
    kind: 'keyTopic',
    keyHex: normalizeKeyHexForMesh(id.keyHex),
    topicId: String(id.topicId).trim()
  }
}
