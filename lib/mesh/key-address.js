 ...
 * direct-pool primary identity (`MeshIdentifier` kind `key`). Topic meshes pass the same topic
...
const { parseSubnet } = require('../ip/ip-subnet')
...
function recomputeL4ChecksumIpv4 (packet) {
  ...
  const l4Min = proto === PROTO_TCP ? 20 : 8
  if (segmentLen < l4Min) return

  const srcIp = packet.subarray(12, 16)
  const dstIp = packet.subarray(16, 20)
  const csumOffset = proto === PROTO_TCP ? 16 : 6
  const csumAbs = IPV4_HEADER_LEN + csumOffset
  if (csumOffset + 2 > segmentLen || packet.length < csumAbs + 2) return

  packet.writeUInt16BE(0, csumAbs)
  ...
  packet.writeUInt16BE(csum, csumAbs)
}
...
function ipv4BufAllZero (b) {
  return (b[0] | b[1] | b[2] | b[3]) === 0
}

function ipv4BufIsRfc1918 (b) {
  const a = b[0]
  const x = b[1]
  if (a === 10) return true
  if (a === 172 && x >= 16 && x <= 31) return true
  if (a === 192 && x === 168) return true
  return false
}

/**
 * Rewrite src only when it looks like a wrong **local** / private source. Packets from the exit
 * host’s TUN carrying internet return traffic use a **global** src (real remote) that is not in `ka`;
 * rewriting those to `localTunIpStr` would break TCP (SYN-ACK would appear from the VPN IP).
 */
function ipv4SrcShouldCoerceToLocalMesh (srcBuf) {
  if (ipv4BufAllZero(srcBuf)) return true
  if (ipv4BufIsRfc1918(srcBuf)) return true
  if (srcBuf[0] === 127) return true
  return false
}

/**
 * Before IPv4 key-address encode, ensure source IP is in `ka` when it should be “ours”. Some stacks
 * (notably macOS) inject packets on the TUN with source set to a LAN address instead of the mesh TUN IP;
 * rewrite src to `localTunIpStr` and fix IPv4 + L4 checksums. **Global** source addresses are left
 * unchanged so full-tunnel reply relay from the exit host preserves the real remote IP.
 *
 * @param {Buffer} packet
...
function coerceIpv4SourceForMeshEncode (packet, ka, localTunIpStr) {
  ...
  try {
    ...
    const srcBuf = packet.subarray(12, 16)
    if (!ipv4SrcShouldCoerceToLocalMesh(srcBuf)) {
      return packet
    }
 ...
 *   datagram (not wire); null if corrupt or if the first nibble is not IPv4/IPv6 (do not send to
 *   TUN). IPv6 passes through raw.
 */
  ...
  if (v === VER_IPV4) {
    if (framedPayload.length >= IPV4_WIRE_HEADER_LEN_MIN) {
      try {
        ...
        console.error('[ka-decode-fail] len=' + framedPayload.length + ' minLen=' + IPV4_WIRE_HEADER_LEN_MIN + ' err=' + (e && e.message) + ' hdr=' + Buffer.from(framedPayload.subarray(0, Math.min(20, framedPayload.length))).toString('hex'))
      }
    }
    if (looksLikeRawIpv4Datagram(framedPayload)) {
      return Buffer.from(framedPayload)
    }
    console.error('[ka-decode-fail] rawCheck=false len=' + framedPayload.length + ' hdr=' + Buffer.from(framedPayload.subarray(0, Math.min(20, framedPayload.length))).toString('hex'))
    return null
  }
  // Not IPv4/IPv6 (e.g. 0x00-prefixed in-band control). Do not pass through — callers would
  // `tun.write` it and the kernel returns EINVAL.
  return null
}
