...
const { encode, createDecoder, startKeepalive } = require('../wire/framing')
const {
...
} = require('../route/routing')
const { createPeerIpAllocator } = require('../ip/ip-subnet')
const { meshIdentifierStorageKey } = require('../wire/mesh-identifier')
const { encodeZ32 } = require('../wire/key-encoding')

/**
 * One TUN + one IPv4 pool: Hyperswarm {@link Hyperswarm#joinPeer} for direct key dials
 * over length-framed key-address payloads. Uses {@link Hyperswarm#listen} so peers can
 * connect to this public key.
 *
 * @param {object} opts
 ...
 * @param {function(string): string|null} [opts.takeReservedPeerIp] — if set, called with 64-hex remote key before allocating an alias; returned IP is {@link createPeerIpAllocator#claim}'d (e.g. mesh IP reservations / SpoonDNS path)
 * @param {function(object): void} [opts.attachSharedSwarmTunnel] — when `swarm` is injected, called instead of attaching a local `data` decoder ({@link ControlPlaneSessionManager} provides one inbound pipeline)
 * @param {boolean} [opts.tunQuiet] — if true, do not print TUN “device up” line (control plane)
 */
function createDirectPool (opts) {
  ...
  const attachSharedSwarmTunnel =
    !ownSwarm && typeof opts.attachSharedSwarmTunnel === 'function'
      ? opts.attachSharedSwarmTunnel
      : null
  let keyPair
  if (ownSwarm) {
  ...
  const tun = createTunDevice({
    ipv4,
    ipv6,
    mtu,
    quiet: opts.tunQuiet === true
  })
  ...
  const takeReservedPeerIp =
    opts.takeReservedPeerIp && typeof opts.takeReservedPeerIp === 'function'
      ? opts.takeReservedPeerIp
      : null

  function pickPeerIpForKey (keyHex) {
    const h = String(keyHex || '').trim().toLowerCase()
    if (takeReservedPeerIp) {
      const want = takeReservedPeerIp(h)
   ...
   * mesh peer IP are forwarded to this peer’s stream for full-tunnel egress. Without this,
   * internet-bound packets are dropped because they do not resolve in the key-address table.
   */
  ...
  tun.on('data', function (packet) {
    ...
    if (!connection || connection.destroyed) {
      if (destIp != null && shouldHairpinToLocalStack(destIp, routingCtx)) {
        try {
          tun.write(packet)
        } catch (_) {}
      }
  ...
  function attachSwarmConnection (st, conn, primeOpts) {
    ...
    conn.opened.then(function () {
      ...
      if (attachSharedSwarmTunnel) {
        attachSharedSwarmTunnel({ conn, st, primeOpts })
      } else {
      ...
      conn.__nospoonDirectPoolCloseHook = true
  ...
  /**
   * Topic-first path never ran {@link #acceptConnection} / {@link #joinPeer}; create primary
   * `kind: 'key'` row in `ka` + {@link PeerState} so primary TUN egress can `ipToKey(dst)`.
   */
  function ensurePrimaryPeerSlotForSharedStream (conn) {
    if (!conn || !conn.remotePublicKey || conn.destroyed) return
    const rh = conn.remotePublicKey.toString('hex')
    if (peers.has(rh)) return
    const meshId = { kind: 'key', keyHex: rh }
    if (ka.ipForMeshIdentifier(meshId)) return
    ...
    try {
      ka.register(peerIp, conn.remotePublicKey, meshId)
    } catch (_) {
      peerIpAllocator.release(peerIp)
      return
    }
  ...
  /**
   * Single entry for control plane: primary router + optional new peer slot + connected state.
   * @param {import('stream').Duplex} conn
   */
  function syncSharedInboundToPrimary (conn) {
    if (!conn || !conn.remotePublicKey || conn.destroyed) return
    router.addPeer(conn.remotePublicKey, conn)
    ensurePrimaryPeerSlotForSharedStream(conn)
    attachSharedStreamToPeerState(conn)
  }

  /**
   * When the control plane attaches an already-open stream to the primary router (e.g. topic-first
   * shared Hyperswarm), promote {@link PeerState} from `connecting` → `connected` so {@link #listPeers}
   * matches the live tunnel. Registers `close` cleanup if {@link #attachSwarmConnection} did not.
   *
   * @param {import('stream').Duplex} conn
   */
  function attachSharedStreamToPeerState (conn) {
    if (!conn || !conn.remotePublicKey || conn.destroyed) return
    const rh = conn.remotePublicKey.toString('hex')
    const st = peers.get(rh)
    if (!st || st.leaving) return
    if (st.connection === conn && st.status === 'connected') return

    st.connection = conn
    st.err = null
    st.status = 'connected'

    if (!conn.__nospoonDirectPoolCloseHook) {
      conn.__nospoonDirectPoolCloseHook = true
      conn.on('close', function () {
        const stNow = peers.get(rh)
        if (!stNow || stNow.connection !== conn) return
        stNow.connection = null
        safeRemovePeer(rh)
        if (stNow.leaving) return
        stNow.status = 'connecting'
        stNow.err = null
        if (stNow.hooks && typeof stNow.hooks.onStreamClose === 'function') {
          try {
            stNow.hooks.onStreamClose(conn)
          } catch (_) {}
        }
        if (opts.policyHooks && typeof opts.policyHooks.onStreamClose === 'function') {
          try {
            opts.policyHooks.onStreamClose(rh, conn)
          } catch (_) {}
        }
        notifyPeersChange()
      })
    ...
    if (opts.policyHooks && typeof opts.policyHooks.onStreamOpen === 'function') {
      try {
        opts.policyHooks.onStreamOpen(rh, conn)
      } catch (_) {}
    }
  }
