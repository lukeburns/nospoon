  ...
  try {
    ...
    switch (msg.method) {
      ...
      case 'libp2pStatus':
        ...
        break
      case 'meshHeliaDialStatus':
        result = backend.meshHeliaDialStatus()
        break
...
;(async function boot () {
  try {
    parentPort.postMessage({ op: 'boot_progress', stage: 'worker_start' })
    ...
    const heliaOpts =
      ...
      typeof workerData.helia === 'object'
        ? workerData.helia
        : {}
    parentPort.postMessage({ op: 'boot_progress', stage: 'helia_modules' })
    const { createHeliaBackend } = require('./ipfs-resolve-content')
    parentPort.postMessage({ op: 'boot_progress', stage: 'helia_starting' })
