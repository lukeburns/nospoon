 ...
 * @param {function(Buffer): boolean} [opts.deliverLocalMeshPacket] — inner IP pkt for another local TUN (shared swarm); return true if handled
 */
function createDirectPool (opts) {
  ...
  const deliverLocalMeshPacket =
    opts.deliverLocalMeshPacket && typeof opts.deliverLocalMeshPacket === 'function'
      ? opts.deliverLocalMeshPacket
      : null
  ...
  function attachSwarmConnection (st, conn, primeOpts) {
    ...
    conn.opened.then(function () {
      ...
      const onTunnelData = function (data) {
        if (st.connection !== conn) return
        decode(data)
      }
      conn.on('data', onTunnelData)
      st._tunnelOnData = onTunnelData
  ...
  /**
   * Shared Hyperswarm: only one stack may attach `data` to a peer stream. Mute ours so topic mesh can own inbound (or vice versa).
   * @param {string} keyHex — 64 hex
   */
  function mutePeerInboundData (keyHex) {
    const h = String(keyHex || '').trim().toLowerCase()
    const st = peers.get(h)
    if (!st || !st._tunnelOnData || !st.connection) return
    st.connection.removeListener('data', st._tunnelOnData)
    st._tunnelInboundMuted = true
  }
