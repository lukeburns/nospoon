/**
 * v86 (fetch backend) + browser-net: each inbound mesh TCP session is bridged to a
 * guest TCP port (SSH by default). See v86 examples/tcp_terminal.html.
 */

/** Bump if snapshot format or guest config (RAM/VGA/v86) changes and old blobs must be ignored. */
...
const IDB_NAME = 'nospoon-v86-hello'
const IDB_VER = 1
const IDB_STORE = 'kv'
...
/**
 * SeaBIOS/VGA BIOS and FreeBSD disk chunks are served from /v86/guest/* (see copy-v86-assets.js).
 * Run `npm run fetch-freebsd-disk` once for the ~2 GiB image. Cold boot does not use copy.sh’s zstd
 * snapshot (it targets their UI defaults: ne2k, ACPI); save/resume via IndexedDB uses this build’s config.
 * @param {{ controlPanelOrigin: string }} opts
 */
export async function initV86HelloDemo (opts) {
  ...
  try {
    ...
    const shimUrl = new URL('/browser-net-shim.js', originBase).href
    shimMod = await import(shimUrl)
  } catch (e) {
    log('v86 init failed (check /v86/* and browser-net-shim): ' + (e && e.message ? e.message : e))
    return
  }

  /** BIOS/VGA + FreeBSD HDA from control panel (see copy-v86-assets.js). */
  ...
  /** Same disk layout as copy.sh freebsd / freebsd-boot — 2 GiB in 1 MiB chunks. */
  ...
  const cu = new URL(originBase)
  shimMod.setBrowserNetProxy({
    hostname: cu.hostname,
    port: cu.port ? Number(cu.port) : undefined,
    pathname: '/api/browser-net'
  })
  const wsUrl = shimMod.defaultBrowserNetWsUrl(window.location.href)
  ...
  function v86BaseConfig (initialStateBuf) {
    const c = {
      ...
      /** copy.sh v86 defaults to ne2k; virtio breaks their zstd restore and mismatches cold boot. */
  ...
  startSshBtn.onclick = async function () {
    ...
    const guestPort = Number(guestPortEl.value) || 22
    ...
    const port = Number(sshPortEl.value) || 2222
    const host =
      bindHostEl && bindHostEl.value.trim() ? bindHostEl.value.trim() : ''
    const server = new shimMod.BrowserNetServer({ url: wsUrl })
