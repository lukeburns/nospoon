...
/**
 * Verify an inner IP packet on a Noise-bound peer stream is sourced from that peer.
 * IPv4 must match `peerIpv4` (this host’s alias for them) or be registered in `ka` to `peerKeyHex`.
 * IPv6 is accepted: mesh uses v4 aliases only; comparing v6 sources to `peerIpv4` would drop all v6
 * (common Happy Eyeballs / AAAA stalls when the tunnel is IPv4-only).
 */
