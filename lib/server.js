...
async function startServer ({ ip = '10.0.0.1/24', ipv6, seed, mtu = 1400, config, fullTunnel, outInterface, allowedKeys, dht: dhtOpt }) {
  const seedBuf = seed
    ...
    : Buffer.from(loadOrCreatePersistedSeedHex(), 'hex')

  if (config && Array.isArray(allowedKeys) && allowedKeys.length) {
    throw new Error('Use either config or allowedKeys, not both')
  }
  ...
  console.log('')
  console.log('Hub public key (clients: nospoon.createClient({ key: <hex>, ... })):')
  console.log('  ' + encodeZ32(keyPair.publicKey))
