...
// to a different relay). Called when the connection's remote
// address changes.
...
/** Remove one host exemption without restoring split routes or DNS. */
function removeHostExemption (remoteHost) {
  if (!remoteHost || !savedGateway) return
  const idx = savedRemoteHosts.indexOf(remoteHost)
  if (idx === -1) return
  const gw = savedGateway
  run('ip', ['route', 'del', remoteHost + '/32', 'via', gw.gateway, 'dev', gw.device], { strict: false })
  savedRemoteHosts.splice(idx, 1)
}
