...
class ControlPlaneSessionManager extends EventEmitter {
  ...
  /**
   * One Hyperswarm stream → one `data` reader. Mute topic decoders before handing inbound to the primary pool.
   * @param {string} peerKeyHex
   */
  _muteAllTopicInboundForPeerKey (peerKeyHex) {
    const rk = String(peerKeyHex || '').trim().toLowerCase()
    if (rk.length !== 64) return
    for (const row of this._topics.values()) {
      const h = row._handle
      if (h && typeof h.muteInboundDataForPeerKey === 'function') {
        h.muteInboundDataForPeerKey(rk)
      }
    }
  }
   ...
   * @param {string} peerKeyHex
   */
  _muteDirectPoolInboundForPeerKey (peerKeyHex) {
    if (!this._directPool || typeof this._directPool.mutePeerInboundData !== 'function') return
    const rk = String(peerKeyHex || '').trim().toLowerCase()
    if (rk.length !== 64) return
    this._directPool.mutePeerInboundData(rk)
  }
  ...
  async _demuxUnknownInbound (conn) {
    ...
    if (conn.isInitiator) {
      if (conn.remotePublicKey) {
        this._muteAllTopicInboundForPeerKey(conn.remotePublicKey.toString('hex'))
      }
  ...
  async initDirectPool () {
    ...
    this._directPool = createDirectPool({
      ...
      deliverLocalMeshPacket (packet) {
        return self._deliverLocalMeshPacket(packet)
      },
