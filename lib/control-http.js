...
class ControlPlaneSessionManager extends EventEmitter {
  ...
  constructor (opts = {}) {
    ...
    this._browserNetProxy = createBrowserNetProxy({
      ...
      resolveListenBind (hostRaw) {
        return selfSm.resolveBrowserNetListenBind(hostRaw)
      },
      resolveConnectHost (hostRaw) {
        return selfSm.resolveBrowserNetConnectHost(hostRaw)
      },
      getOutboundRoute (remoteIpv4) {
        return selfSm.getBrowserNetOutboundRoute(remoteIpv4)
      }
    })
   ...
   */
  resolveBrowserNetListenBind (hostRaw) {
    const s = String(hostRaw || '').trim()
    if (!s) return null
    if (net.isIPv4(s)) {
      return this._isLocalMeshTunIp(s) ? s : null
    }
    ...
    const ip = this.dnsResolveMeshIpv4(meshId)
   ...
   * @returns {string | null}
   */
  resolveBrowserNetConnectHost (hostRaw) {
    const s = String(hostRaw || '').trim()
    if (!s) return null
   ...
   */
  getBrowserNetOutboundRoute (remoteIpv4) {
  ...
  /**
   * @returns {{ enabled: boolean, mode: string, dataDir: string, externalGatewayUrl: string, listening: boolean, ipv4: string | null, httpPort: number, lastError: string | null }}
   */
  getIpfsDwebStatus () {
  ...
  /**
   * List root DAG pins (panel uploads and anything else pinned as a root in Helia).
  ...
  async listIpfsSeeds () {
    ...
    const b = this._ipfsHeliaBackend
    if (!b || typeof b.listSeeds !== 'function') {
      return []
    }
    return b.listSeeds()
  }
  ...
  /**
   * Unpin a root CID and run garbage collection (embedded Helia only).
   * @param {string} cidStr
   */
  async removeIpfsSeed (cidStr) {
    if (!this._ipfsConfig.enabled) {
      throw new Error('IPFS gateway is disabled')
    }
    if (this._ipfsConfig.mode !== 'helia') {
      throw new Error('Unseed requires embedded Helia')
    }
    const b = this._ipfsHeliaBackend
    if (!b || typeof b.unseed !== 'function') {
      throw new Error('Helia is not ready yet (enable DNS and IPFS, then wait for startup)')
    }
    await b.unseed(String(cidStr || '').trim())
  }
  ...
  async _syncIpfsDwebService () {
    ...
    const { resolveViaExternalGateway, createHeliaBackend } = require('./ipfs-resolve-content')
    ...
    try {
      if (this._ipfsConfig.mode === 'external') {
        ...
        this._ipfsHeliaBackend = await createHeliaBackend(dir)
...
async function startControlHttpServer (opts = {}) {
  ...
  const server = http.createServer(async function (req, res) {
    ...
    try {
      ...
      if (req.method === 'GET' && path === '/api/ipfs/seeds') {
        sessions
          .listIpfsSeeds()
          .then(function (seeds) {
            sendJson(res, 200, { seeds })
          })
          .catch(function (e) {
            const msg = e && e.message ? e.message : String(e)
      ...
      if (req.method === 'DELETE' && path === '/api/ipfs/seeds') {
        let cid = ''
        try {
          const u = new URL(req.url || '', 'http://127.0.0.1')
          cid = String(u.searchParams.get('cid') || '').trim()
        } catch (_) {}
        if (!cid) {
          sendJson(res, 400, { error: 'missing cid query parameter' })
          return
        }
        sessions
          .removeIpfsSeed(cid)
