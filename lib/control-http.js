...
const { nospoonHeliaSwarmPort } = require('./ipfs-resolve-content')
...
/**
 * Max raw body size for `POST /api/ipfs/add` (streams to a temp file; not held in memory).
...
async function streamRequestToFile (req, filePath, maxBytes) {
  ...
  const limiter = new Transform({
    transform (chunk, _enc, cb) {
      ...
      if (written > maxBytes) {
        cb(new Error('upload too large (max ' + maxBytes + ' bytes)'))
        return
      }
      cb(null, chunk)
    }
