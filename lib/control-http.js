...
const { createDecoder } = require('./framing')
...
class ControlPlaneSessionManager extends EventEmitter {
  ...
  async _ensureSharedSwarm () {
    ...
    this._sharedSwarm.on('update', function () {
      if (self._swarmStatusDebounceTimer) clearTimeout(self._swarmStatusDebounceTimer)
      self._swarmStatusDebounceTimer = setTimeout(function () {
  ...
  async _demuxUnknownInbound (conn) {
    ...
    const payload = await readFirstNonKeepalivePayload(conn)
...
function readFirstNonKeepalivePayload (conn) {
  return new Promise(function (resolve, reject) {
    const decode = createDecoder(function (pkt) {
