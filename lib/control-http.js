...
const { loadOrCreatePersistedSeedHex } = require('./nospoon-identity')
const {
...
const { startKeepalive, encode: frameEncode } = require('./framing')
...
/** Max age for CID auth cookies (seconds). */
const CID_AUTH_TTL = 86400
/**
...
/** Max wait for a topic row during browser-net WebSocket {@code prepare} (competing tabs / fast reloads). */
const BROWSER_NET_TOPIC_JOIN_WAIT_MS = 90_000
...
/**
 * @param {{ clientSeedHex?: string | null }} [opts]
 * @returns {string}
 */
...
class ControlPlaneSessionManager extends EventEmitter {
  /**
   * @param {{ primaryCidr?: string | null, clientSeedHex?: string | null, ipfsEnabled?: boolean, systemDnsOverride?: boolean, darwinSystemDns?: boolean }} [opts]
   */
  constructor (opts = {}) {
    super()
    ...
    /** @type {Buffer | null} — lazily derived CID auth HMAC key */
    this._cidAuthSecretBuf = null
    ...
    /** @type {{ enabled: boolean, mode: 'helia' | 'external', dataDir: string, externalGatewayUrl: string, heliaDhtClientMode: boolean }} */
    this._ipfsConfig = {
      enabled: opts.ipfsEnabled !== false,
      mode: 'helia',
      dataDir: '',
    ...
    /** @type {{ stop: function(): Promise<void>, resolveContent: function(o: object): Promise<object>, libp2pStatus?: function(): Promise<object>, initialHeliaLibp2p?: object | null } | null} */
    this._ipfsHeliaBackend = null
    /** @type {object | null} */
    this._ipfsHeliaLibp2pCache = null
    /** @type {boolean} */
    ...
    /**
     * Last `meshHeliaDialStatus` snapshot from the Helia worker (mesh IP:port → learned PeerId).
     * @type {{ targets: Array<{ targetKey: string, meshIpv4: string, port: number, learnedPeerId: string | null }> } | null}
     */
    this._ipfsHeliaMeshDialCache = null
    /** Human-readable Helia worker boot status for the control panel (embedded mode). */
    this._ipfsHeliaBootProgress = null
    /** JSON snapshot `{ mesh: string[], primary: string|null }` at last Helia worker start. */
    this._ipfsHeliaListenContextAtStart = null
    /** Last resolved default-route swarm IPv4 (for status UI). */
    this._ipfsHeliaPrimarySwarmPreview = null
    /** @type {ReturnType<typeof setTimeout> | null} */
    this._heliaMeshResyncTimer = null
    this._ipfsDwebSyncRunning = false
    this._ipfsDwebSyncRerun = false
    /**
     * Mutable pointer so CID dweb HTTP can keep the same {@code resolveContent} while swapping
     * the Helia worker (mesh listener resync without tearing down :80 or browser-net WSS).
     * @type {{ backend: { resolveContent: function(object): Promise<object>, stop: function(): Promise<void> } | null }}
     */
    this._heliaResolveTarget = { backend: null }
    /** Serialize embedded Helia worker swaps from {@link #_resyncEmbeddedHeliaListeners}. */
    this._heliaEmbedResyncRunning = false
    this._heliaEmbedResyncRerun = false
    /** One-shot from {@link #_resolveHeliaDwebContent} when dweb is up but Helia backend was lost. */
    this._ipfsDwebOrphanRecoverScheduled = false
    const selfSm = this
    this._browserNetProxy = createBrowserNetProxy({
      ...
      resolveDns (hostname, ws) {
        return selfSm.resolveBrowserNetDns(hostname, ws)
      },
      reverseDns (ip, ws) {
        return selfSm.resolveBrowserNetReverseDns(ip, ws)
      }
    })
  ...
  /**
   * Like {@link #_ensureTopicJoinedForDnsAwait} but for browser-net: wait out an in-flight
   * {@link #addTopic} so rapid reloads / parallel tabs do not hit
   * {@code this topic join is already in progress} and fail the WebSocket.
   * @param {string} topicRef
   */
  async _ensureTopicJoinedForBrowserNet (topicRef) {
    const t = String(topicRef || '').trim()
    if (!t) return
    const discoveryHex = swarmDiscoveryKey(t).toString('hex')
    const deadline = Date.now() + BROWSER_NET_TOPIC_JOIN_WAIT_MS
    while (Date.now() < deadline) {
      if (this._resolveTopicRowByRef(t)) return
      if (this._pendingTopicDiscoveryHex.has(discoveryHex)) {
        await new Promise(function (r) {
    ...
    if (!this._resolveTopicRowByRef(t)) {
      throw new Error('browser-net: timeout waiting for topic join: ' + t)
    }
  }
  ...
  /**
   * Reverse DNS (PTR): given an IPv4, return the FQDN that would resolve to it, or null.
   * Priority: manual registry → self primary → direct pool peers → primary reservations →
   * self topic TUN → topic live peers → topic reservations.
   * @param {string} ip
   * @returns {string | null}
   */
  _dnsReverseLookupPtr (ip) {
    const target = String(ip || '').trim()
    if (!target) return null

    const manual = this._dnsManualRegistry.reverseLookupIpv4(target)
    if (manual) return manual

    const selfHex = this._clientKeyPair.publicKey.toString('hex')

    if (target === this._dnsPrimaryTunHostIpv4()) {
      return formatKeyToDnsName(selfHex)
    }

    if (this._directPool) {
      for (const p of this._directPool.listPeers()) {
        if (p.peerAliasIp === target) {
          return formatKeyToDnsName(p.keyHex)
        }
      }
    }

    const reserved = this._meshIpReservations.reverseLookupIpv4(target)
    if (reserved) {
      if (reserved.kind === 'key') {
        return formatKeyToDnsName(reserved.keyHex)
      }
      if (reserved.kind === 'keyTopic') {
        const row = this._topics.get(reserved.topicId)
        const topicLabel = row ? row.topic : reserved.topicId
        return formatMeshTopicDnsName(reserved.keyHex, topicLabel)
      }
    }

    for (const row of this._topics.values()) {
      if (target === row.localTunIp) {
        return formatMeshTopicDnsName(selfHex, row.topic)
      }
      for (const [keyHex, peerObj] of row._peers) {
        if (peerObj && peerObj.ipv4 === target) {
          return formatMeshTopicDnsName(keyHex, row.topic)
        }
      }
    }
  ...
  /**
   * Lazily derive a stable HMAC key from the node's private key.
   * @returns {Buffer}
   */
  _cidAuthSecret () {
    if (!this._cidAuthSecretBuf) {
      this._cidAuthSecretBuf = crypto
        .createHmac('sha256', this._clientKeyPair.secretKey)
        .update('nospoon-cid-auth')
        .digest()
    }
    return this._cidAuthSecretBuf
  }

  /**
   * Mint a Set-Cookie header value granting topic authority for {@code cidStr}.
   * @param {string} cidStr
   * @returns {string}
   */
  _mintCidAuthCookie (cidStr) {
    const iat = Math.floor(Date.now() / 1000).toString(16)
    const topic = String(cidStr).toLowerCase()
    const mac = crypto
      .createHmac('sha256', this._cidAuthSecret())
      .update(iat + '.' + topic)
      .digest('hex')
    const value = iat + '.' + topic + '.' + mac
    return '__nospoon_cid=' + value + '; HttpOnly; SameSite=Strict; Path=/; Max-Age=' + CID_AUTH_TTL
  }

  /**
   * Verify a {@code Cookie} request header and extract the granted topic.
   * @param {string | undefined} cookieHeader — raw {@code req.headers.cookie}
   * @returns {{ topic: string } | null}
   */
  _verifyCidAuthCookie (cookieHeader) {
    if (!cookieHeader) return null
    const prefix = '__nospoon_cid='
    const cookies = String(cookieHeader).split(';')
    let raw = null
    for (let i = 0; i < cookies.length; i++) {
      const c = cookies[i].trim()
      if (c.startsWith(prefix)) {
        raw = c.substring(prefix.length)
        break
      }
    }
    if (!raw) return null
  ...
  async prepareBrowserNetWebSocket (ws, req) {
    ...
    const cidAuth = this._verifyCidAuthCookie(req && req.headers && req.headers.cookie)
    if (cidAuth) {
      const topicNorm = normalizeFqdn(cidAuth.topic)
      ws._browserNetOriginRestricted = true
      ws._browserNetOriginTopicRef = topicNorm
      await this._ensureTopicJoinedForBrowserNet(cidAuth.topic)
      return
    }
    ...
    if (this._isLoopbackBrowserNetOriginHost(host)) {
      ...
      try {
        ...
        const devTopic = reqUrl.searchParams.get('topic')
        if (devTopic && devTopic.trim()) {
          await this._ensureTopicJoinedForBrowserNet(devTopic.trim())
        }
      } catch (_) {}
      return
    }
    ...
    if (meshFromOrigin && meshFromOrigin.kind === 'keyTopic') {
      ...
      ws._browserNetOriginRestricted = true
      ws._browserNetOriginTopicRef = topicNorm
      await this._ensureTopicJoinedForBrowserNet(topicForJoin)
      return
    }
  ...
  /**
   * Bridge-scoped DNS: resolve a hostname for a browser guest. Only names
   * within the WebSocket's bound topic are allowed — prevents enumeration
   * and forced joins. Bypasses _scheduleTopicJoinFromDns.
   * @param {string} hostname
   * @param {import('ws')} ws
   * @returns {string | null}
   */
  resolveBrowserNetDns (hostname, ws) {
    const name = String(hostname || '').trim().toLowerCase()
    if (!name) return null
    const meshId = parseMeshDnsName(name)
    if (!meshId) return null
    const topicRef = ws && ws._browserNetOriginTopicRef
    if (meshId.kind === 'key') {
      if (topicRef) return null
      return this.dnsResolveMeshIpv4(meshId)
    }
    if (topicRef && meshId.topicRef !== topicRef) return null
    return this.dnsResolveMeshIpv4(meshId)
  }

  /**
   * Bridge-scoped reverse DNS: IPv4 → hostname. Delegates to
   * _dnsReverseLookupPtr and filters to the socket's topic scope.
   * @param {string} ip
   * @param {import('ws')} ws
   * @returns {string | null}
   */
  resolveBrowserNetReverseDns (ip, ws) {
    const hostname = this._dnsReverseLookupPtr(ip)
    if (!hostname) return null
    const topicRef = ws && ws._browserNetOriginTopicRef
    if (!topicRef) return hostname
    const meshId = parseMeshDnsName(hostname)
    if (!meshId) return null
    if (meshId.kind === 'key') return null
    if (meshId.topicRef !== topicRef) return null
    return hostname
  }
  ...
  getDnsStatus () {
    ...
    if (this._systemDnsOverride) {
      const o = require('./dns-system-override')
      systemDnsOverride = {
        enabled: true,
        active: o.isActive(),
  ...
  /**
   * @returns {{ enabled: boolean, mode: string, dataDir: string, externalGatewayUrl: string, heliaDhtClientMode: boolean, listening: boolean, ipv4: string | null, httpPort: number, lastError: string | null, canUpload: boolean, heliaLibp2p: object | null, heliaMeshPeers: Array<{ hyperKeyHex: string, hyperKeyZ32: string, ipfsPeerId: string, meshIpv4: string, port: number, primaryMultiaddr: string, multiaddrs: string[], connected: boolean }>, browserNetDweb: { listening: boolean, ipv4: string | null, wsPort: number, dnsHost: string, lastError: string | null } }}
   */
  getIpfsDwebStatus () {
    ...
    return {
      ...
      heliaDhtClientMode: this._ipfsConfig.heliaDhtClientMode === true,
      externalGatewayUrl:
      ...
      heliaLibp2p:
        this._ipfsConfig.mode === 'helia' ? this._ipfsHeliaLibp2pCache : null,
      heliaMeshPeers: this.getHeliaMeshPeersStatus(),
      heliaBootProgress:
        this._ipfsConfig.mode === 'helia' ? this._ipfsHeliaBootProgress : null,
      heliaMeshListenIps:
        this._ipfsConfig.mode === 'helia' ? this._heliaMeshListenIpv4s() : [],
      heliaPrimarySwarmIpv4:
        this._ipfsConfig.mode === 'helia'
          ? this._ipfsHeliaPrimarySwarmPreview
          : null,
      browserNetDweb: {
  ...
  /**
   * Peers with an active shared Hyperswarm tunnel: mesh alias IP + libp2p PeerId (from dial handshake when known).
   * @returns {Array<{ hyperKeyHex: string, hyperKeyZ32: string, ipfsPeerId: string, meshIpv4: string, port: number, primaryMultiaddr: string, multiaddrs: string[], connected: boolean, meshHeliaConnectState: 'connected' | 'connecting' | 'handshake_pending' | 'disconnected', meshHeliaLastDialError: string | null }>}
   */
  getHeliaMeshPeersStatus () {
    if (this._ipfsConfig.mode !== 'helia') return []
    const cache = this._ipfsHeliaLibp2pCache
    /** @type {Set<string>} */
    const connected = new Set(
      cache && Array.isArray(cache.connectedRemotePeerIds)
        ? cache.connectedRemotePeerIds
        : []
    )
    const nodeMajor =
      cache && typeof cache.nodeMajor === 'number' ? cache.nodeMajor : 0
    const defaultPort = nospoonHeliaSwarmPort()
    /** @type {Map<string, string>} */
    const learnedByTarget = new Map()
    /** @type {Map<string, { dialInFlight: boolean, lastDialError: string | null }>} */
    const dialMetaByTarget = new Map()
    const dialSnap = this._ipfsHeliaMeshDialCache
    if (dialSnap && Array.isArray(dialSnap.targets)) {
      for (let i = 0; i < dialSnap.targets.length; i++) {
        const t = dialSnap.targets[i]
        if (t == null || !t.meshIpv4) continue
        const tk = `${t.meshIpv4}:${t.port | 0}`
        dialMetaByTarget.set(tk, {
          dialInFlight: t.dialInFlight === true,
          lastDialError:
            t.lastDialError != null && String(t.lastDialError).trim() !== ''
              ? String(t.lastDialError)
              : null
        })
        if (t.learnedPeerId == null) continue
        const pid = String(t.learnedPeerId).trim()
        if (!pid) continue
        learnedByTarget.set(tk, pid)
      }
    }
    /** @type {Array<{ hyperKeyHex: string, hyperKeyZ32: string, ipfsPeerId: string, meshIpv4: string, port: number, primaryMultiaddr: string, multiaddrs: string[], connected: boolean, meshHeliaConnectState: 'connected' | 'connecting' | 'handshake_pending' | 'disconnected', meshHeliaLastDialError: string | null }>} */
    const rows = []
    /** @type {Array<{ peerId: string, remoteIp4: string | null }>} */
    const connHints =
      cache && Array.isArray(cache.meshHeliaConnHints)
        ? cache.meshHeliaConnHints
        : []
    for (const [hyperKeyHex, ent] of this._sharedInboundByPeerKey) {
      if (!ent || !ent.conn || ent.conn.destroyed) continue
      const meshIpv4 = String(
        ent.heliaMeshDialIpv4 || ent.peerAliasIp || ''
      ).trim()
      if (!meshIpv4 || !net.isIPv4(meshIpv4)) continue
      const port = defaultPort
      /** @type {string[]} */
      const candidateMeshIps = []
      const pushCand = function (ip) {
        const s = String(ip || '').trim()
        if (net.isIPv4(s) && candidateMeshIps.indexOf(s) === -1) {
          candidateMeshIps.push(s)
        }
      }
      pushCand(ent.heliaMeshDialIpv4)
      pushCand(ent.peerAliasIp)
      pushCand(
        this._heliaMeshDialIpv4ForPeer(hyperKeyHex, ent.peerAliasIp, undefined)
      )
      let ipfsPeerId = ''
      for (let ci = 0; ci < candidateMeshIps.length; ci++) {
        const pid =
          learnedByTarget.get(`${candidateMeshIps[ci]}:${port}`) || ''
      ...
      if (!ipfsPeerId) {
        for (let hi = 0; hi < connHints.length; hi++) {
          const h = connHints[hi]
          if (h == null || !h.peerId) continue
          const rip = h.remoteIp4 != null ? String(h.remoteIp4).trim() : ''
          if (!net.isIPv4(rip) || candidateMeshIps.indexOf(rip) === -1) {
            continue
          }
          const pid = String(h.peerId).trim()
          if (pid) {
            ipfsPeerId = pid
            break
          }
      ...
      let lastErr = null
      for (let ci = 0; ci < candidateMeshIps.length; ci++) {
        const dm = dialMetaByTarget.get(`${candidateMeshIps[ci]}:${port}`)
        if (!dm) continue
        if (dm.dialInFlight) dialInFlight = true
        if (dm.lastDialError != null) lastErr = lastErr || dm.lastDialError
      }
      const isConnected = ipfsPeerId !== '' && connected.has(ipfsPeerId)
      if (isConnected) {
        dialInFlight = false
        lastErr = null
      }
      /** @type {'connected' | 'connecting' | 'handshake_pending' | 'disconnected'} */
      let meshHeliaConnectState
      if (isConnected) meshHeliaConnectState = 'connected'
      else if (dialInFlight) meshHeliaConnectState = 'connecting'
      else if (ipfsPeerId !== '') meshHeliaConnectState = 'disconnected'
      else meshHeliaConnectState = 'handshake_pending'
      const maTcp = ipfsPeerId
        ? `/ip4/${meshIpv4}/tcp/${port}/p2p/${ipfsPeerId}`
        : `/ip4/${meshIpv4}/tcp/${port}`
      const multiaddrs = [maTcp]
      if (nodeMajor >= 22) {
        multiaddrs.push(
          ipfsPeerId
            ? `/ip4/${meshIpv4}/udp/${port}/quic-v1/p2p/${ipfsPeerId}`
            : `/ip4/${meshIpv4}/udp/${port}/quic-v1`
        )
      }
      let hyperKeyZ32 = ''
      try {
        hyperKeyZ32 = encodeZ32(Buffer.from(hyperKeyHex, 'hex'))
      } catch (_) {
        continue
      }
      rows.push({
        ...
        meshIpv4,
        port,
        primaryMultiaddr: maTcp,
        multiaddrs,
        connected: isConnected,
        meshHeliaConnectState,
        meshHeliaLastDialError: lastErr
      })
    }
    rows.sort(function (a, b) {
      return a.hyperKeyZ32.localeCompare(b.hyperKeyZ32)
    })
    return rows
  }

  /**
   * Queue embedded Helia to dial the peer's mesh IPv4 + swarm port. No PeerId is required;
   * libp2p learns it from the secure handshake.
   * @param {string} meshIpv4
   * @param {number|undefined} [port] — omitted uses the worker's default Helia swarm port
   */
  _queueMergeMeshHeliaPeer (meshIpv4, port) {
    const b = this._ipfsHeliaBackend
    if (
      !b ||
      this._ipfsConfig.mode !== 'helia' ||
      typeof b.mergeMeshHeliaPeer !== 'function'
    ) {
      return
    }
    const ip = String(meshIpv4 || '').trim()
    if (!ip) return
    const self = this
    const payload = { meshIpv4: ip }
    if (port != null && Number.isFinite(Number(port))) {
      payload.port = Number(port)
    }
    void b
      .mergeMeshHeliaPeer(payload)
      .catch(function () {})
      .then(function () {
        return self.refreshHeliaLibp2pCache()
      })
      .then(function () {
        self._emitStatus()
      })
  ...
  /**
   * Refresh {@link ControlHttpSessions#_ipfsHeliaLibp2pCache} from the Helia worker (embedded mode).
   * @returns {Promise<void>}
   */
  async refreshHeliaLibp2pCache () {
    try {
      if (this._ipfsConfig.mode === 'helia') {
        this._ipfsHeliaPrimarySwarmPreview =
          await nospoonHeliaSwarmIpv4ListenHost()
      }
    } catch (_) {}
    const b = this._ipfsHeliaBackend
    if (
      !b ||
      this._ipfsConfig.mode !== 'helia' ||
      typeof b.libp2pStatus !== 'function'
    ) {
      this._ipfsHeliaLibp2pCache = null
      this._ipfsHeliaMeshDialCache = null
      return
    ...
    try {
      this._ipfsHeliaLibp2pCache = await b.libp2pStatus()
    } catch (_) {
      // keep previous cache
    }
    try {
      if (typeof b.meshHeliaDialStatus === 'function') {
        this._ipfsHeliaMeshDialCache = await b.meshHeliaDialStatus()
      }
    } catch (_) {
      // keep previous mesh dial cache
    }
  }

  /**
   * Re-run embedded Helia mesh dial for a peer's mesh IPv4 (must match an active shared inbound tunnel).
   * @param {{ meshIpv4?: string, port?: number }} body
   * @returns {Promise<{ ok: true }>}
   */
  async triggerMeshHeliaMeshDial (body) {
    if (!this._ipfsConfig.enabled) {
      throw new Error('IPFS gateway is disabled')
    }
    if (this._ipfsConfig.mode !== 'helia') {
      throw new Error('mesh Helia dial applies only to embedded Helia mode')
    }
    const b = this._ipfsHeliaBackend
    if (!b || typeof b.mergeMeshHeliaPeer !== 'function') {
      throw new Error('Helia is not ready yet')
    }
    const ip = String(body && body.meshIpv4 != null ? body.meshIpv4 : '')
      .trim()
    if (!ip || !net.isIPv4(ip)) {
      throw new Error('meshIpv4 must be a valid IPv4 address')
    }
    let allowed = false
    for (const [, ent] of this._sharedInboundByPeerKey) {
      if (!ent || !ent.conn || ent.conn.destroyed) continue
      const alias = String(ent.peerAliasIp || '').trim()
      const dialTarget = String(ent.heliaMeshDialIpv4 || '').trim()
      if (alias === ip || dialTarget === ip) {
        allowed = true
        break
      }
    }
    if (!allowed) {
      throw new Error('no active mesh tunnel for that address')
    }
    const payload = { meshIpv4: ip }
    if (body.port != null && Number.isFinite(Number(body.port))) {
      payload.port = Number(body.port)
    }
    await b.mergeMeshHeliaPeer(payload)
    await this.refreshHeliaLibp2pCache()
    this._emitStatus()
    return { ok: true }
  }

  /**
   * Stream a file already on disk into Helia (embedded worker). The worker deletes `absPath` after import.
   * @param {string} filename
   * @param {string} absPath
   * @returns {Promise<{ cid: string, filename: string }>}
   */
  async addIpfsFileFromPath (filename, absPath) {
    if (!this._ipfsConfig.enabled) {
      throw new Error('IPFS gateway is disabled')
    }
    if (this._ipfsConfig.mode !== 'helia') {
    ...
    const b = this._ipfsHeliaBackend
    if (!b || typeof b.addFileFromPath !== 'function') {
      throw new Error('Helia is not ready yet (enable DNS and IPFS, then wait for startup)')
    }
    const cid = await b.addFileFromPath({
      filename: String(filename || 'upload'),
      absPath: String(absPath || '').trim()
    })
    return { cid, filename: String(filename || 'upload').replace(/[/\\]/g, '_') || 'upload' }
  }

  /**
   * Add a directory tree: each file is already on disk under `uploadRoot` (worker deletes the tree).
   * @param {string} uploadRoot
   * @param {Array<{ path: string, part: string }>} entries — UnixFS relative path + part basename under uploadRoot
   * @returns {Promise<{ cid: string, filename: string }>}
   */
  async addIpfsDirectoryFromPaths (uploadRoot, entries) {
    if (!this._ipfsConfig.enabled) {
      throw new Error('IPFS gateway is disabled')
    }
    if (this._ipfsConfig.mode !== 'helia') {
    ...
    const b = this._ipfsHeliaBackend
    if (!b || typeof b.addDirectoryFromPaths !== 'function') {
      throw new Error('Helia is not ready yet (enable DNS and IPFS, then wait for startup)')
    }
    return b.addDirectoryFromPaths({
      uploadRoot: String(uploadRoot || '').trim(),
      entries
    })
  }
  ...
  async _stopDnsServer () {
    if (this._systemDnsOverride) {
      const dnsOverride = require('./dns-system-override')
      let restoreOpts
      if (dnsOverride.isActive()) {
        const { parseForwardTarget } = require('./dns-server')
  ...
  async _syncDnsServer () {
    ...
    this._dnsServer = createDnsServer({
      ...
      reverseLookupPtr: function (ip) {
        return self._dnsReverseLookupPtr(ip)
      },
    ...
    try {
      ...
      this._dnsLastError = null
      if (this._systemDnsOverride) {
        const { apply } = require('./dns-system-override')
        const r = apply({
          port: this._dnsConfig.port,
          address: this._dnsConfig.address
        })
      ...
      const { flushSystemDnsCache } = require('./dns-cache-flush')
      flushSystemDnsCache()
    } catch (e) {
      this._dnsLastError = e && e.message ? e.message : String(e)
      this._dnsServer = null
  ...
  /**
   * CID dweb {@code resolveContent} for embedded Helia — reads {@link #_heliaResolveTarget} so the
   * worker can be swapped without recreating the HTTP server.
   * @param {object} o
   * @returns {Promise<object>}
   */
  _resolveHeliaDwebContent (o) {
    const b = this._heliaResolveTarget && this._heliaResolveTarget.backend
    if (!b) {
      if (
        this._ipfsConfig.mode === 'helia' &&
        this._ipfsConfig.enabled &&
        this._ipfsDwebListening &&
        this._ipfsDwebStop &&
        !this._ipfsHeliaBackend &&
        !this._heliaEmbedResyncRunning &&
        !this._ipfsDwebOrphanRecoverScheduled
      ) {
        const self = this
        this._ipfsDwebOrphanRecoverScheduled = true
        setImmediate(function () {
          self._ipfsDwebOrphanRecoverScheduled = false
          void self._syncIpfsDwebServiceAfterIdle()
        })
      }
      return Promise.resolve({
        status: 503,
        headers: { 'Content-Type': 'text/plain; charset=utf-8' },
        body: Buffer.from('ipfs gateway restarting\n', 'utf8')
      })
    }
    return b.resolveContent(o)
  }
   ...
   * Coalesces overlapping calls (e.g. mesh TUN resync while a sync is in flight).
   * @returns {Promise<void>}
   */
  async _syncIpfsDwebService () {
    if (this._ipfsDwebSyncRunning) {
      this._ipfsDwebSyncRerun = true
      return
    }
    this._ipfsDwebSyncRunning = true
    try {
      do {
        this._ipfsDwebSyncRerun = false
        await this._syncIpfsDwebServiceOnce()
      } while (this._ipfsDwebSyncRerun)
    } finally {
      this._ipfsDwebSyncRunning = false
    }
  }
  ...
  /**
   * Wait for any in-flight {@link #_syncIpfsDwebService}, then run a full sync (used for recovery).
   * @returns {Promise<void>}
   */
  async _syncIpfsDwebServiceAfterIdle () {
    const deadline = Date.now() + 120000
    while (this._ipfsDwebSyncRunning && Date.now() < deadline) {
      await new Promise(function (r) {
        setTimeout(r, 25)
      })
    }
   ...
   * @returns {Promise<void>}
   */
  async _syncIpfsDwebServiceOnce () {
    ...
    let resolveContent
    /** Resolved on the control plane for primary TCP/QUIC (default-route LAN); passed into the worker. */
    let heliaPrimarySwarmIpv4 = null
    try {
      if (this._ipfsConfig.mode === 'external') {
        ...
        const selfBoot = this
        try {
          heliaPrimarySwarmIpv4 = await nospoonHeliaSwarmIpv4ListenHost()
        } catch (_) {
          heliaPrimarySwarmIpv4 = null
        }
        this._ipfsHeliaPrimarySwarmPreview = heliaPrimarySwarmIpv4
        this._ipfsHeliaBootProgress = 'Spawning Helia worker…'
        this._emitStatus()
        this._ipfsHeliaBackend = await createHeliaWorkerBackend(
          dir,
          {
            dhtClientMode: this._ipfsConfig.heliaDhtClientMode === true,
            extraListenIpv4: this._heliaMeshListenIpv4s(),
            swarmListenIpv4: heliaPrimarySwarmIpv4
          },
          {
            onBootProgress: function (stage) {
              const labels = {
                worker_start: 'Helia worker started; loading modules…',
                helia_modules: 'Loading Helia (disk I/O + JS deps; often slow on Linux cold start)…',
                helia_starting: 'Starting Helia / libp2p (transports, DHT, listeners)…'
              }
              selfBoot._ipfsHeliaBootProgress =
                labels[stage] || String(stage || 'Starting Helia…')
              selfBoot._emitStatus()
            }
          }
        )
        const backend = this._ipfsHeliaBackend
        this._ipfsHeliaLibp2pCache = backend.initialHeliaLibp2p || null
        void this.refreshHeliaLibp2pCache()
        this._ipfsHeliaBootProgress = 'Starting CID HTTP gateway…'
        this._emitStatus()
        this._heliaResolveTarget.backend = this._ipfsHeliaBackend
        resolveContent = function (o) {
          return selfBoot._resolveHeliaDwebContent(o)
        }
      }
    } catch (e) {
      this._ipfsDwebLastError = e && e.message ? e.message : String(e)
    ...
    const dweb = createIpfsDwebHttpServer({
      ...
      port: 80,
      resolveContent,
      mintAuthCookie: function (cidStr) { return self._mintCidAuthCookie(cidStr) }
    })
    let httpSrv = dweb
    try {
      await dweb.start()
      this._ipfsDwebStop = async function () {
        ...
        self._heliaResolveTarget.backend = null
        const hb = self._ipfsHeliaBackend
        if (hb) {
          try {
            await hb.stop()
          } catch (_) {}
        ...
        self._ipfsHeliaLibp2pCache = null
        self._ipfsHeliaMeshDialCache = null
      }
      this._ipfsDwebListening = true
      this._ipfsDwebBindIpv4 = bindIpv4
      this._ipfsHeliaBootProgress = null
      if (this._ipfsConfig.mode === 'helia') {
        this._ipfsHeliaListenContextAtStart = JSON.stringify({
          mesh: this._heliaMeshListenIpv4s().slice().sort(),
          primary: heliaPrimarySwarmIpv4
        })
      }
      try {
      ...
      this._ipfsHeliaBootProgress = null
      this._heliaResolveTarget.backend = null
      const hbFail = this._ipfsHeliaBackend
      if (hbFail) {
        try {
          await hbFail.stop()
        } catch (_) {}
  ...
  async applyIpfsSettings (body) {
    ...
    if (typeof body.heliaDhtClientMode === 'boolean') {
      this._ipfsConfig.heliaDhtClientMode = body.heliaDhtClientMode
    }
  ...
  _deliverLocalMeshPacket (packet) {
    ...
    if (this._directPool && dest === this._directPool.localTunIp) {
      console.error('[deliver] dest=' + dest + ' → primary TUN')
      try {
    ...
    for (const row of this._topics.values()) {
      if (dest === row.localTunIp) {
        const h = row._handle
        console.error('[deliver] dest=' + dest + ' → topic=' + row.topic + ' tun=' + !!(h && h.tun))
        if (h && h.tun) {
          try {
            h.tun.write(packet)
          } catch (_) {}
    ...
    console.error('[deliver] dest=' + dest + ' → NO MATCH (topics: ' + Array.from(this._topics.values()).map(function (r) { return r.topic + '=' + r.localTunIp }).join(', ') + ')')
    return false
  }
  ...
  /**
   * IPv4 to use for embedded Helia mesh TCP/QUIC dials: prefer primary (direct pool / reservation) over topic-tunnel alias.
   * @param {string} keyHex — 64 hex remote public key
   * @param {string} topicTunnelAliasIp — Hyperswarm tunnel framing IP (topic subnet when on topic-only path)
   * @param {{ peerIp?: string }|null|undefined} directSt — when set, {@code peerIp} is the live primary alias
   * @returns {string}
   */
  _heliaMeshDialIpv4ForPeer (keyHex, topicTunnelAliasIp, directSt) {
    const fallback = String(topicTunnelAliasIp || '').trim()
    let rk
    try {
      rk = normalizeKeyHex(String(keyHex || '').trim())
    } catch (_) {
      return fallback
    }
    if (!/^[0-9a-f]{64}$/.test(rk)) return fallback
    if (directSt != null && directSt.peerIp != null) {
      const d = String(directSt.peerIp).trim()
      if (d && net.isIPv4(d)) return d
    }
    if (this._directPool && typeof this._directPool.listPeers === 'function') {
      for (const p of this._directPool.listPeers()) {
        if (p && p.keyHex === rk && p.peerAliasIp) {
          const ip = String(p.peerAliasIp).trim()
          if (net.isIPv4(ip)) return ip
        }
      }
    }
    const reserved = this._meshIpReservations.getPrimaryReservedIpv4ForKey(rk)
    if (reserved != null) {
      const ip = String(reserved).trim()
      if (net.isIPv4(ip)) return ip
    }
    return fallback
  }
  ...
  _attachSharedPeerInbound (args) {
    ...
    if (prev && prev.conn === conn) {
      if (args.directSt) {
        ...
        const nextHelia = this._heliaMeshDialIpv4ForPeer(
          rk,
          prev.peerAliasIp,
          args.directSt
        )
        const prevDial = String(
          prev.heliaMeshDialIpv4 || prev.peerAliasIp || ''
        ).trim()
        if (net.isIPv4(nextHelia) && nextHelia !== prevDial) {
          prev.heliaMeshDialIpv4 = nextHelia
          const selfUp = this
          queueMicrotask(function () {
            selfUp._queueMergeMeshHeliaPeer(nextHelia, undefined)
          })
        }
      }
      return
    ...
    const heliaMeshDialIpv4 = this._heliaMeshDialIpv4ForPeer(
      rk,
      peerAliasIp,
      args.directSt
    )
    ...
    if (args.topicPreauthedInbound) {
      ...
      const selfPre = this
      const dialIp = heliaMeshDialIpv4
      queueMicrotask(function () {
        selfPre._queueMergeMeshHeliaPeer(dialIp, undefined)
      })
    }
    ...
    if (args.topicWireInbound) {
      ...
      topicFirstFrameAuth = {
        ...
        onSuccess: function () {
          ...
          const dialIp = self._heliaMeshDialIpv4ForPeer(
            rk,
            peerAliasIp,
            args.directSt
          )
          const cur = self._sharedInboundByPeerKey.get(rk)
          if (cur && cur.conn === conn) cur.heliaMeshDialIpv4 = dialIp
          queueMicrotask(function () {
            self._queueMergeMeshHeliaPeer(dialIp, undefined)
          })
        },
    ...
    const push = createSharedSwarmInboundPush({
      ...
      tryBrowserProxy: function (packet, ctx) {
        const consumed = self._browserNetProxy.tryConsumeInboundPacket(packet, ctx)
        if (consumed && packet.length >= 20) console.error('[proxy-consumed] dest=' + packet[16] + '.' + packet[17] + '.' + packet[18] + '.' + packet[19])
        return consumed
      }
    })

    const onData = function (chunk) {
      if (chunk.length > 4) console.error('[hs-ingress] from=' + rk.slice(0, 8) + ' len=' + chunk.length)
      push(chunk)
    }
    conn.on('data', onData)
    ...
    this._sharedInboundByPeerKey.set(rk, {
      conn,
      onData,
      push,
      peerAliasIp,
    ...
    if (!topicFirstFrameAuth && !args.topicPreauthedInbound) {
      this._startKeepaliveOnce(conn)
      const selfDirect = this
      const dialIp = heliaMeshDialIpv4
      queueMicrotask(function () {
        selfDirect._queueMergeMeshHeliaPeer(dialIp, undefined)
      })
    }
  }
  ...
  async initDirectPool () {
    ...
    this._maybeResyncHeliaForMeshListeners()
  }
  ...
  /**
   * Spoon / topic TUN IPv4s for extra libp2p listeners (in addition to the default-route primary bind).
   * @returns {string[]}
   */
  _heliaMeshListenIpv4s () {
    const seen = new Set()
    /** @type {string[]} */
    const out = []
    const push = function (ip) {
      const s = String(ip || '').trim()
      if (!net.isIPv4(s) || seen.has(s)) return
      seen.add(s)
      out.push(s)
    }
    if (this._directPool && this._directPool.localTunIp) {
      push(this._directPool.localTunIp)
    }
    for (const row of this._topics.values()) {
      if (row && row.localTunIp) push(row.localTunIp)
    }
    return out
  }

  /**
   * Primary default-route swarm bind + mesh TUN extras (for detecting when to restart embedded Helia).
   * @returns {Promise<string>}
   */
  async _heliaEmbeddedListenContextSnapshot () {
    let primary = null
    try {
      primary = await nospoonHeliaSwarmIpv4ListenHost()
    } catch (_) {
      primary = null
    }
  ...
  /**
   * Replace the Helia worker with one bound to the current mesh TUN + primary listen snapshot.
   * Keeps CID dweb HTTP and {@link #_browserNetDwebWss} running.
   * @returns {Promise<void>}
   */
  async _swapEmbeddedHeliaWorkerForListenContext () {
    const dir =
      String(this._ipfsConfig.dataDir || '').trim() ||
      nodePath.join(os.homedir(), '.nospoon', 'helia')
    const selfBoot = this
    ...
    const old = this._ipfsHeliaBackend
    /** Keep serving via {@code old} until {@code newBackend} is ready — avoids null-backend 503 / stuck gateway if spawn fails or is slow. */
    this._ipfsHeliaBootProgress = 'Spawning Helia worker…'
    this._emitStatus()
    const { createHeliaWorkerBackend } = require('./ipfs-helia-worker-bridge')
    let newBackend
    try {
      newBackend = await createHeliaWorkerBackend(
        dir,
        {
          dhtClientMode: this._ipfsConfig.heliaDhtClientMode === true,
          extraListenIpv4: this._heliaMeshListenIpv4s(),
        ...
        },
        {
          onBootProgress: function (stage) {
            const labels = {
              worker_start: 'Helia worker started; loading modules…',
              helia_modules:
                'Loading Helia (disk I/O + JS deps; often slow on Linux cold start)…',
              helia_starting: 'Starting Helia / libp2p (transports, DHT, listeners)…'
            }
            selfBoot._ipfsHeliaBootProgress =
              labels[stage] || String(stage || 'Starting Helia…')
    ...
    this._ipfsHeliaBackend = newBackend
    this._heliaResolveTarget.backend = newBackend
    this._ipfsHeliaLibp2pCache = newBackend.initialHeliaLibp2p || null
    this._ipfsHeliaMeshDialCache = null
    void this.refreshHeliaLibp2pCache()
    this._ipfsHeliaBootProgress = null
    this._ipfsHeliaListenContextAtStart = JSON.stringify({
  ...
  /**
   * Embedded Helia only: if mesh/primary listen context changed, swap the worker without tearing down
   * CID HTTP :80 or browser-net on {@link BROWSER_NET_DWEB_WS_PORT}.
   * @returns {Promise<void>}
   */
  async _resyncEmbeddedHeliaListeners () {
    if (this._ipfsConfig.mode !== 'helia' || !this._ipfsConfig.enabled) return
    if (!this._ipfsDwebListening || !this._ipfsDwebStop) return
    if (!this._ipfsHeliaBackend) {
      await this._syncIpfsDwebServiceAfterIdle()
      return
    }

    if (this._heliaEmbedResyncRunning) {
      this._heliaEmbedResyncRerun = true
      return
    }
    this._heliaEmbedResyncRunning = true
    try {
      do {
        this._heliaEmbedResyncRerun = false
        const c2 = await this._heliaEmbeddedListenContextSnapshot()
        if (c2 === this._ipfsHeliaListenContextAtStart) continue
        try {
          await this._swapEmbeddedHeliaWorkerForListenContext()
        } catch (e) {
          this._ipfsDwebLastError = e && e.message ? e.message : String(e)
          await this._syncIpfsDwebServiceAfterIdle()
        }
      } while (this._heliaEmbedResyncRerun)
    } finally {
      this._heliaEmbedResyncRunning = false
    }
    this._emitStatus()
  ...
  /**
   * When the default-route IPv4 or mesh TUN addresses change after Helia started, restart the IPFS dweb
   * service so libp2p picks up the right primary + extra listeners.
   * @returns {void}
   */
  _maybeResyncHeliaForMeshListeners () {
    if (this._ipfsConfig.mode !== 'helia' || !this._ipfsConfig.enabled) return
    if (!this._ipfsHeliaBackend || !this._ipfsDwebListening) return
    if (this._heliaMeshResyncTimer != null) {
      clearTimeout(this._heliaMeshResyncTimer)
      this._heliaMeshResyncTimer = null
    }
    const self = this
    this._heliaMeshResyncTimer = setTimeout(function () {
      self._heliaMeshResyncTimer = null
      void (async function () {
        if (self._ipfsConfig.mode !== 'helia' || !self._ipfsConfig.enabled) return
        if (!self._ipfsHeliaBackend || !self._ipfsDwebListening) return
        const c2 = await self._heliaEmbeddedListenContextSnapshot()
        if (c2 === self._ipfsHeliaListenContextAtStart) return
        await self._resyncEmbeddedHeliaListeners()
        self._emitStatus()
      })()
    }, 450)
    if (typeof this._heliaMeshResyncTimer.unref === 'function') {
      this._heliaMeshResyncTimer.unref()
    }
  }
  ...
  getTopicSnapshot (id) {
    ...
    const swarm = this._sharedSwarm
    const peerInfoByHex = new Map()
    if (swarm) {
      for (const [, pi] of swarm.peers) {
        if (pi.publicKey) peerInfoByHex.set(pi.publicKey.toString('hex'), pi)
      }
    }
    row.peers = merged.map((p) => {
      ...
      const pi = peerInfoByHex.get(p.peerKeyHex)
      const liveOnTopic = pi
        ? discoveryIncludedInPeerTopics(pi.topics, discoveryBuf)
        : false
      return { ...p, meshDnsWireName, liveOnTopic }
    })
    return {
      id: row.id,
      topic: row.topic,
...
/** Former in-memory panel cap; if still set in env it breaks large streaming uploads. */
const LEGACY_IPFS_PANEL_MAX_BYTES = 64 * 1024 * 1024

/**
 * Max raw body size for `POST /api/ipfs/add` and total bytes for `POST /api/ipfs/add-directory` (streamed to disk).
 * Override with `NOSPOON_IPFS_MAX_UPLOAD_BYTES` (integer bytes). Default 512 GiB.
 * @returns {number}
 */
function nospoonIpfsStreamUploadMaxBytes () {
  const raw = process.env.NOSPOON_IPFS_MAX_UPLOAD_BYTES
  if (raw == null || String(raw).trim() === '') {
    return 512 * 1024 * 1024 * 1024
  }
  const n = Number(String(raw).trim())
  if (!Number.isFinite(n) || n < 1) return 512 * 1024 * 1024 * 1024
  if (n === LEGACY_IPFS_PANEL_MAX_BYTES) {
    console.warn(
      '[nospoon] NOSPOON_IPFS_MAX_UPLOAD_BYTES is 64 MiB (old in-memory limit). Ignoring it; streaming uploads default to 512 GiB. Unset the variable or set a larger cap.'
    )
    return 512 * 1024 * 1024 * 1024
  }
...
const MAX_IPFS_STREAM_UPLOAD_BYTES = nospoonIpfsStreamUploadMaxBytes()

/**
 * Stream request body to a file with a byte cap (O(disk) memory).
 * @param {import('http').IncomingMessage} req
  ...
  const limiter = new Transform({
    transform (chunk, _enc, cb) {
      ...
      if (written > maxBytes) {
        cb(
          new Error(
            'upload too large (max ' +
              maxBytes +
              ' bytes; unset or raise NOSPOON_IPFS_MAX_UPLOAD_BYTES)'
          )
        )
        return
      }
...
/**
 * Parse multipart/form-data: stream each file part to `uploadRoot/part-XXXXXXXX.bin`.
 * Field name (or filename=) is normalized as the UnixFS relative path.
 * @param {import('http').IncomingMessage} req
 * @param {string} uploadRoot — existing directory
 * @param {number} maxTotalBytes — sum of all part sizes
 * @returns {Promise<Array<{ path: string, part: string }>>}
 */
function readMultipartIpfsDirectoryToDisk (req, uploadRoot, maxTotalBytes) {
  return new Promise(function (resolve, reject) {
    ...
    /** @type {Array<{ path: string, part: string }>} */
    const items = []
    const seen = new Set()
    let total = 0
    let partSeq = 0
    ...
    const bb = busboy({
      ...
      limits: {
        files: 100_000,
        // Per-part cap: allow any single file up to the same budget as the whole upload.
    ...
    bb.on('file', function (fieldname, file, info) {
      ...
      let rawName = String(fieldname || '').trim()
      if (rawName === 'f' || rawName === '') {
        rawName = info && info.filename ? String(info.filename).trim() : ''
      }
      if (!rawName) {
        file.resume()
        return
      ...
      let unixPath
      try {
        unixPath = normalizeUnixfsRelPath(rawName)
      } catch (_) {
        file.resume()
        return
      }
      if (seen.has(unixPath)) {
        fail(new Error('duplicate path: ' + unixPath))
        file.resume()
        return
      }
      seen.add(unixPath)
      partSeq += 1
      if (partSeq > 99_999_999) {
        fail(new Error('too many files in folder upload'))
        file.resume()
        return
      }
      const part = 'part-' + String(partSeq).padStart(8, '0') + '.bin'
      const absPart = nodePath.join(uploadRoot, part)
      file.once('limit', function () {
        fail(
          new Error(
            'folder upload: a single file exceeds the max part size (' +
              maxTotalBytes +
              ' bytes; unset or raise NOSPOON_IPFS_MAX_UPLOAD_BYTES)'
          )
        )
      })
      work = work.then(async function () {
        if (settled) {
          file.resume()
          return
        }
            ...
            if (total > maxTotalBytes) {
              cb(
                new Error(
                  'upload too large (max ' +
                    maxTotalBytes +
                    ' bytes total; unset or raise NOSPOON_IPFS_MAX_UPLOAD_BYTES)'
                )
              )
              return
            }
        ...
        await pipeline(file, limiter, fs.createWriteStream(absPart))
        if (!settled) {
          items.push({ path: unixPath, part })
        }
      })
    })
...
/**
 * @param {{ port?: number, host?: string, primaryCidr?: string | null, ipfsEnabled?: boolean, systemDnsOverride?: boolean, darwinSystemDns?: boolean }} [opts] — omit `host` to bind on an auto loopback alias with manual name {@link CONTROL_PANEL_DNS_HOST} (port defaults to 80). Set `ipfsEnabled: false` to skip embedded Helia / CID dweb until enabled in the UI. By default, macOS/Linux point the OS resolver at 127.0.0.1 while mesh DNS runs (`networksetup` or `resolvectl` / `/etc/resolv.conf`; prepend `.` to search domains on macOS and via `resolvectl` on Linux). Disable with `systemDnsOverride: false` or env `NOSPOON_SYSTEM_DNS=0` (legacy: `NOSPOON_DARWIN_SYSTEM_DNS=0`). Restored on shutdown.
...
async function startControlHttpServer (opts = {}) {
  ...
  const server = http.createServer(async function (req, res) {
    ...
    try {
      ...
      if (req.method === 'GET' && path === '/how-nospoon') {
        res.writeHead(200, HTML_TYPE)
        res.end(controlPageHtml())
        return
      }
      ...
      if (path === '/api/browser-log' || path === '/api/browser-cmd') {
        ...
        if (path === '/api/browser-log' && req.method === 'POST') {
          let body = ''
          req.on('data', function (c) { body += c })
          req.on('end', function () {
            try { const msgs = JSON.parse(body); if (Array.isArray(msgs)) msgs.forEach(function (m) { console.error('[browser] ' + m) }) } catch (_) {}
            res.writeHead(204, cors); res.end()
          })
          return
        }
        if (path === '/api/browser-cmd' && req.method === 'GET') {
          if (!server._pendingBrowserCmds) server._pendingBrowserCmds = []
          const cmds = server._pendingBrowserCmds.splice(0)
          res.writeHead(200, Object.assign({ 'Content-Type': 'application/json' }, cors))
          res.end(JSON.stringify(cmds))
          return
        }
        if (path === '/api/browser-cmd' && req.method === 'POST') {
          let body = ''
          req.on('data', function (c) { body += c })
          req.on('end', function () {
            try {
              ...
              if (!server._pendingBrowserCmds) server._pendingBrowserCmds = []
              server._pendingBrowserCmds.push(cmd)
              console.error('[browser-cmd] queued: ' + JSON.stringify(cmd))
            } catch (_) {}
            res.writeHead(204, cors); res.end()
          })
          return
      ...
      if (req.method === 'POST' && path === '/api/ipfs/mesh-dial') {
        let body = {}
        try {
      ...
      if (req.method === 'DELETE' && path.startsWith('/api/ipfs/pins/')) {
        ...
        sessions
          .unpinIpfs(cid)
          .then(async function () {
            await sessions.refreshHeliaLibp2pCache()
      ...
      if (req.method === 'POST' && path === '/api/ipfs/add') {
        ...
        const tmpPath = nodePath.join(
          os.tmpdir(),
          'nospoon-ipfs-add-' + crypto.randomBytes(16).toString('hex') + '.bin'
        )
        try {
          const n = await streamRequestToFile(
            req,
            tmpPath,
            MAX_IPFS_STREAM_UPLOAD_BYTES
          )
          if (!n) {
            try {
          ...
          const out = await sessions.addIpfsFileFromPath(filename, tmpPath)
          await sessions.refreshHeliaLibp2pCache()
          sessions._emitStatus()
          sendJson(res, 200, out)
        } catch (e) {
      ...
      if (req.method === 'POST' && path === '/api/ipfs/add-directory') {
        ...
        const uploadRoot = nodePath.join(
          os.tmpdir(),
          'nospoon-ipfs-dir-' + crypto.randomBytes(16).toString('hex')
        )
        try {
          fs.mkdirSync(uploadRoot, { recursive: true })
          const items = await readMultipartIpfsDirectoryToDisk(
            req,
            uploadRoot,
            MAX_IPFS_STREAM_UPLOAD_BYTES
          )
          if (!items.length) {
            ...
            sendJson(res, 400, { error: 'no files selected' })
            return
          }
          const out = await sessions.addIpfsDirectoryFromPaths(uploadRoot, items)
          await sessions.refreshHeliaLibp2pCache()
          sessions._emitStatus()
          sendJson(res, 200, out)
        } catch (e) {
