...
const { encodeHeliaMeshAnnounce } = require('./ipfs-mesh-announce')
const {
...
function heliaMeshAnnounceEnabled () {
  const v = process.env.NOSPOON_HELIA_MESH_ANNOUNCE
...
class ControlPlaneSessionManager extends EventEmitter {
  ...
  constructor (opts = {}) {
    ...
    /** @type {WeakSet<import('stream').Duplex>} */
    this._heliaMeshAnnounceSent = new WeakSet()
    /**
     * Hyperswarm peer key (hex) → last in-tunnel Helia announce from that peer (for control UI).
     * @type {Map<string, { hyperKeyHex: string, hyperKeyZ32: string, meshIpv4: string, port: number, ipfsPeerId: string, updatedAt: number }>}
     */
    this._meshHeliaAnnounceByHyperKey = new Map()
    const selfSm = this
  ...
  async _stopDnsServer () {
    if (this._systemDnsOverride) {
      const { restore } = require('./dns-system-override')
      restore()
    }
  ...
  _attachSharedPeerInbound (args) {
    ...
    const push = createSharedSwarmInboundPush({
      ...
      onHeliaMeshAnnounce: function (ann, swarmCtx) {
        ...
        const b = self._ipfsHeliaBackend
        const p =
          !b ||
          self._ipfsConfig.mode !== 'helia' ||
          typeof b.mergeMeshHeliaPeer !== 'function'
            ? Promise.resolve()
            : b.mergeMeshHeliaPeer({
                meshIpv4: peerAliasIp,
                peerIdStr: ann.peerId,
                port: ann.port
              })
        void p
          .catch(function () {})
