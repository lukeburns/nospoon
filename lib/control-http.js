...
class ControlPlaneSessionManager extends EventEmitter {
  ...
  /**
   * Local mesh TUN IPv4s for Helia: primary direct pool + each topic interface.
   * Used so libp2p listens on `/ip4/<tun>/tcp/<swarm>` (4011) for mesh dials, not only WebSocket on ephemeral ports.
   * @returns {string[]}
   */
