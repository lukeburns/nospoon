...
class ControlPlaneSessionManager extends EventEmitter {
  ...
  getHeliaMeshPeersStatus () {
    ...
    for (const [hyperKeyHex, ent] of this._sharedInboundByPeerKey) {
      ...
      const meshIpv4 = String(ent.peerAliasIp || '').trim()
      if (!meshIpv4 || !net.isIPv4(meshIpv4)) continue
      const port = defaultPort
      const tk = `${meshIpv4}:${port}`
      const ipfsPeerId = learnedByTarget.get(tk) || ''
      const dm = dialMetaByTarget.get(tk)
      const dialInFlight = dm ? dm.dialInFlight === true : false
      const lastErr = dm && dm.lastDialError ? dm.lastDialError : null
  ...
  _attachSharedPeerInbound (args) {
    ...
    if (args.topicPreauthedInbound) {
      ...
      queueMicrotask(function () {
        selfPre._queueMergeMeshHeliaPeer(peerAliasIp, undefined)
      })
    }
    ...
    if (args.topicWireInbound) {
      ...
      topicFirstFrameAuth = {
        ...
        onSuccess: function () {
          ...
          queueMicrotask(function () {
            self._queueMergeMeshHeliaPeer(peerAliasIp, undefined)
          })
        },
    ...
    if (!topicFirstFrameAuth && !args.topicPreauthedInbound) {
      ...
      queueMicrotask(function () {
        selfDirect._queueMergeMeshHeliaPeer(peerAliasIp, undefined)
      })
    }
  }
