...
class ControlPlaneSessionManager extends EventEmitter {
  ...
  constructor (opts = {}) {
    ...
    /**
     * Last {@link meshHeliaDialStatus} snapshot from the Helia worker (mesh IP:port → learned PeerId).
  ...
  /**
   * Peers that announced Helia over the mesh tunnel: Hyperswarm key → dial multiaddrs + libp2p session state.
   * @returns {Array<{ hyperKeyHex: string, hyperKeyZ32: string, ipfsPeerId: string, meshIpv4: string, port: number, primaryMultiaddr: string, multiaddrs: string[], connected: boolean }>}
   */
  getHeliaMeshPeersStatus () {
    ...
    /** @type {Array<{ hyperKeyHex: string, hyperKeyZ32: string, ipfsPeerId: string, meshIpv4: string, port: number, primaryMultiaddr: string, multiaddrs: string[], connected: boolean }>} */
    const rows = []
    for (const rec of this._meshHeliaAnnounceByHyperKey.values()) {
      const maTcp = `/ip4/${rec.meshIpv4}/tcp/${rec.port}/p2p/${rec.ipfsPeerId}`
      const multiaddrs = [maTcp]
      if (nodeMajor >= 22) {
        multiaddrs.push(
          `/ip4/${rec.meshIpv4}/udp/${rec.port}/quic-v1/p2p/${rec.ipfsPeerId}`
        )
      }
      rows.push({
        hyperKeyHex: rec.hyperKeyHex,
        hyperKeyZ32: rec.hyperKeyZ32,
        ipfsPeerId: rec.ipfsPeerId,
        meshIpv4: rec.meshIpv4,
        port: rec.port,
        primaryMultiaddr: maTcp,
        multiaddrs,
        connected: connected.has(rec.ipfsPeerId)
      })
    }
  ...
  /**
   * @param {{ peerId: string, port: number }} ann
   * @param {{ peerKeyHex: string, peerAliasIp: string }} ctx
   */
  _recordHeliaMeshAnnounceFromSwarm (ann, ctx) {
    const hyperKeyHex = String(ctx.peerKeyHex || '').trim().toLowerCase()
    if (hyperKeyHex.length !== 64 || !/^[0-9a-f]+$/.test(hyperKeyHex)) {
      return
    }
    const meshIpv4 = String(ctx.peerAliasIp || '').trim()
    const ipfsPeerId = String(ann.peerId || '').trim()
    const port = ann.port | 0
    if (!meshIpv4 || !ipfsPeerId || port < 1 || port > 65535) return
    let hyperKeyZ32 = ''
    try {
    ...
    this._meshHeliaAnnounceByHyperKey.set(hyperKeyHex, {
      hyperKeyHex,
      hyperKeyZ32,
      meshIpv4,
      port,
  ...
  async _stopIpfsDwebService () {
    ...
    this._ipfsHeliaBackend = null
    this._ipfsHeliaLibp2pCache = null
    this._meshHeliaAnnounceByHyperKey.clear()
    this._ipfsDwebListening = false
    this._ipfsDwebBindIpv4 = null
  }
  ...
  /**
   * Send local Helia PeerId + swarm TCP port once per shared Hyperswarm stream (length-framed).
   * @param {import('stream').Duplex} conn
   */
  _sendHeliaMeshAnnounceOnce (conn) {
    if (!heliaMeshAnnounceEnabled()) return
    if (!conn || conn.destroyed) return
    if (this._heliaMeshAnnounceSent.has(conn)) return
    if (!this._ipfsHeliaBackend || this._ipfsConfig.mode !== 'helia') return

    const self = this
    function trySend () {
      if (!conn || conn.destroyed || self._heliaMeshAnnounceSent.has(conn)) return true
      const peerId = String(
        (self._ipfsHeliaLibp2pCache && self._ipfsHeliaLibp2pCache.peerId) || ''
      ).trim()
      if (!peerId) return false
      self._heliaMeshAnnounceSent.add(conn)
      try {
        conn.write(
          frameEncode(
            encodeHeliaMeshAnnounce({
              peerId,
              port: nospoonHeliaSwarmPort()
            })
          )
        )
      } catch (_) {}
    ...
    if (trySend()) return
    void this.refreshHeliaLibp2pCache().then(function () {
      trySend()
    })
  }
  ...
  _attachSharedPeerInbound (args) {
    ...
    if (args.topicPreauthedInbound) {
      ...
      queueMicrotask(function () {
        selfPre._sendHeliaMeshAnnounceOnce(conn)
    ...
    if (args.topicWireInbound) {
      ...
      topicFirstFrameAuth = {
        ...
        onSuccess: function () {
          ...
          queueMicrotask(function () {
            self._sendHeliaMeshAnnounceOnce(conn)
    ...
    const push = createSharedSwarmInboundPush({
      ...
      onHeliaMeshAnnounce: function (ann, swarmCtx) {
        self._recordHeliaMeshAnnounceFromSwarm(ann, swarmCtx)
        self._queueMergeMeshHeliaPeer(
          peerAliasIp,
          ann.port != null && Number.isFinite(Number(ann.port))
            ? Number(ann.port)
            : undefined
        )
      }
    ...
    if (!topicFirstFrameAuth && !args.topicPreauthedInbound) {
      ...
      queueMicrotask(function () {
        selfDirect._sendHeliaMeshAnnounceOnce(conn)
