...
const { formatKeyToDnsName, formatMeshTopicDnsName } = require('./dns-mesh-name')
...
class ControlPlaneSessionManager extends EventEmitter {
   ...
   */
  setDnsManualRecord (body) {
    const host = String(body.hostname || body.host || '').trim()
    if (!host) throw new Error('hostname is required')
    this._dnsManualRegistry.set(host, {
      ipv4: body.ipv4,
      ipv6: body.ipv6
    })
    this._emitStatus()
    return this._dnsManualRegistry.list()
  }

  deleteDnsManualRecord (hostname) {
  ...
  _lookupRelayConnection (destIp) {
    ...
    const inferred = this._sharedSwarmLiteralDestSinglePeerConnection(destIp)
    return inferred && !inferred.destroyed ? inferred : null
  }

  /**
   * When the destination is an unregistered 10.x literal (e.g. the peer’s real topic TUN IP on a
   * different /24) but there is exactly one shared-swarm peer, relay on that stream so literals
   * round-trip on the wire. Ambiguous when multiple peers are connected.
   * @param {string} destIp
   * @returns {import('stream').Duplex|null}
   */
  _sharedSwarmLiteralDestSinglePeerConnection (destIp) {
    const ka = this._sharedKeyAddress
    if (!ka || typeof destIp !== 'string' || !destIp.startsWith('10.')) return null
    try {
      ka.ipToKey(destIp)
      return null
    } catch {
      // not keyed — ok to consider literal fallback
    }
    if (this._sharedInboundByPeerKey.size !== 1) return null
    const entry = this._sharedInboundByPeerKey.values().next().value
    const c = entry && entry.conn
  ...
  async addTopic (opts) {
    ...
    try {
      ...
      const handle = await startSwarmMesh({
        ...
        lookupOutboundRelay (destIp) {
          return self._lookupRelayConnection(destIp)
        },
        hooks: {
          onPeerUp (info) {
...
/**
 * @param {{ port?: number, host?: string, primaryCidr?: string | null }} [opts]
  ...
  const server = http.createServer(async function (req, res) {
    ...
    try {
      ...
      if (req.method === 'POST' && path === '/api/dns/manual') {
        const body = await readBody(req)
        const list = sessions.setDnsManualRecord(body)
        sendJson(res, 200, { manual: list })
        return
      }

      if (req.method === 'DELETE' && path.startsWith('/api/dns/manual/')) {
        const host = decodeURIComponent(path.slice('/api/dns/manual/'.length))
        const ok = sessions.deleteDnsManualRecord(host)
        sendJson(res, 200, { ok })
        return
      }
