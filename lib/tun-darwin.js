...
// ioctl is variadic — on ARM64 this changes the calling convention
...
function createUtunSocket () {
  ...
  const ret = ioctlFn(fd, CTLIOCGINFO, 'void *', ctlInfo)
  if (ret < 0) {
    fs.closeSync(fd)
  ...
  addr.writeUInt16LE(AF_SYS_CONTROL, 2) // ss_sysaddr
  addr.writeUInt32LE(ctlId, 4)          // sc_id
  addr.writeUInt32LE(0, 8)              // sc_unit = 0 (auto-assign)
