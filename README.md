...
A peer-to-peer VPN that **does not require a publicly reachable coordinator**. Peers find each other with [HyperDHT](https://github.com/holepunchto/hyperdht) and [Hyperswarm](https://github.com/holepunchto/hyperswarm), punch through NAT, and carry IP packets over **Noise-encrypted** streams. No public IP, no port forwarding, no central infra — only keys (and, in swarm mode, a **topic** you share).

**This branch focuses on:**

| Mode | Command | Idea |
|------|---------|------|
| **HTTP control plane** | `nospoon` (default) or `nospoon web` | Join Hyperswarm topics and direct peers, mesh DNS, TUN — browser UI on `/`. |
| **Topic mesh** | `nospoon swarm <topic>` | Same topic string ⇒ same overlay; pairwise tunnels only. |
...
| `--seed <z32|hex>` | random | Deterministic peer key |
...
1. Peers discover each other via **Hyperswarm** + topic (swarm) or via the **control plane** (direct pool + shared swarm).
