 ...
 * ({@link ../lib/browser-net-proxy.js}).
 *
 * Uses {@link EventTarget}; {@link #on} is a small convenience alias for {@link EventTarget#addEventListener}.
 *
 * ## Future: real `net` polyfill
 * Goal: Browserify/Webpack alias of Node’s `net` like
 * [net-browserify](https://github.com/emersion/net-browserify) — call {@link setBrowserNetProxy} for the
 * WebSocket bridge (`hostname` / `port` of the control server), then reuse code that does
 * `net.connect` / `createServer`. Today we only expose {@link BrowserNetServer} and the binary/JSON
 * protocol; outbound mesh `connect` and full API parity are still to do.
 */

/** Must match {@link BIN_TAG} in browser-net-proxy.js */
