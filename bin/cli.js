...
Usage:
  ...
  nospoon genkey                    Generate a random seed + public key

  'web' / 'control' are aliases for the default command. Use --help for this message.

Hub/spoke (one HyperDHT hub, others dial its key) is available from Node via the package API:
  require('nospoon').startServer({ ... }) and require('nospoon').createClient({ key, ... }).
See README.md and lib/index.js.

Keys and seeds use z32 encoding (32-byte values). 64-character hex is still accepted.

Swarm options:
  --ip <cidr>           TUN IPv4 (default: first free 10.0.x.1/24 on this host; use --ip to fix)
...
Examples:
  ...
  sudo nospoon                          # control plane on port 80
`)
}
